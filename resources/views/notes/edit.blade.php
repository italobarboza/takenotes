...
<form method="POST" action="{{route('notes.save', $note->id)}}">
    ...
    <div class="form-row">
        <div class="col-md-6 mb-3">
            ...
            <input type="text" class="form-control @if ($errors->has('title')) is-invalid @endif" id="title" name="title" placeholder="Title" value="{{ old('title', $note->title) }}">
            @if ($errors->has('title'))
    ...
    <div class="form-group">
        ...
        <textarea class="form-control @if ($errors->has('note')) is-invalid @endif" id="note" name="note" rows="5">{{ old('note', $note->note) }}</textarea>
        @if ($errors->has('note'))
            <span class="invalid-feedback">
